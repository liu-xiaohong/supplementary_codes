...
names(x = split(as.data.frame(Test_expr), f = Test_surv$Cohort)) # ע?????Լ???׼??˳??????һ??
...
write.table(RS_mat, file.path(res.path, "RS_mat.txt"),sep = "\t", row.names = T, col.names = NA, quote = F) # ?????????��??ļ?
...
pdf(file.path(fig.path, "heatmap of cindex.pdf"), width = cellwidth * ncol(Cindex_mat) + 6, height = cellheight * nrow(Cindex_mat) * 0.45)
